...
`vpk pack` passes `--packTitle "Start Launcher"` so the installer's Start Menu entry and Add/Remove Programs display name read "Start Launcher" (with a space) instead of the bare `--packId` value (`StartLauncher`). Velopack's `--shortcuts` defaults to `Desktop,StartMenuRoot`, and it tracks the shortcuts it creates so the generated uninstaller removes them automatically — no app code needed for that cleanup. This is separate from the per-command shortcuts `ExecutablesAccessor` creates under `Start Menu\Programs\Start Launcher\Commands\`, which the app itself owns and which `EnsureCommands()` already reconciles against `commands.json`.
